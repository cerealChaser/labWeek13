package ex6servertexttranslationapp;
...
/**
 * This class wil parse the translated text to the variable where it will
 * be sent back to the client based on the input received and the selected
 * target language from client's request
 * 
 * 
...
public class TextTranslator {
	...
	/**
	 * Create data to be sent for translation
	 */
	public void createData()
	{
			...
			{
				// write in Malay
				// create stream to write Malay translated text to 
				// the Malay text storage
				...
				// write in Arabic
				// create stream to write Arabic translated text 
				// to the Arabic text storage
	...
	public String translateToBM(String text)
	{
		...
		try
		{
			...
			else if(text.toUpperCase().equals("HOW ARE YOU") 
			...
			else if(text.toUpperCase().equals("WHAT'S UP ?") 
	...
	public String translateToKrn(String text)
	{
		...
		try
		{
			...
			// Conditions to parse the correspond text to the 
			// respective variable based on English text input
			if(text.toUpperCase().equals("GOOD MORNING"))
			{
				translated = morning;
			}
